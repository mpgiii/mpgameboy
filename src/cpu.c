...
/* 8-bit sub-register accessors */
static uint8_t get_a(void) { return (cpu.af & A_MASK) >> 8; }
static void set_a(uint8_t v) { cpu.af = (cpu.af & FLAGS_MASK) | ((uint16_t)v << 8); }
static uint8_t get_b(void) { return (cpu.bc & B_MASK) >> 8; }
static void set_b(uint8_t v) { cpu.bc = (cpu.bc & C_MASK) | ((uint16_t)v << 8); }
static uint8_t get_c(void) { return cpu.bc & C_MASK; }
static void set_c(uint8_t v) { cpu.bc = (cpu.bc & B_MASK) | v; }
static uint8_t get_d(void) { return (cpu.de & D_MASK) >> 8; }
static void set_d(uint8_t v) { cpu.de = (cpu.de & E_MASK) | ((uint16_t)v << 8); }
static uint8_t get_e(void) { return cpu.de & E_MASK; }
static void set_e(uint8_t v) { cpu.de = (cpu.de & D_MASK) | v; }
static uint8_t get_h(void) { return (cpu.hl & H_MASK) >> 8; }
static void set_h(uint8_t v) { cpu.hl = (cpu.hl & L_MASK) | ((uint16_t)v << 8); }
static uint8_t get_l(void) { return cpu.hl & L_MASK; }
static void set_l(uint8_t v) { cpu.hl = (cpu.hl & H_MASK) | v; }

/* flag helpers */
static void write_flag(uint8_t flag, int set) {
    if (set) cpu.af |= flag;
    else cpu.af &= ~flag;
}
...
void ld_a_bcp(void) {
    set_a(mem_read8(cpu.bc));
}

void ld_de_nn(void) {
    cpu.de = mem_read16(cpu.pc);
    cpu.pc += 2;
}

void ld_dep_a(void) {
    mem_write8(cpu.de, get_a());
}
...
void ld_a_dep(void) {
    set_a(mem_read8(cpu.de));
}

void ld_hl_nn(void) {
    cpu.hl = mem_read16(cpu.pc);
    cpu.pc += 2;
}

void ldi_hlp_a(void) {
    mem_write8(cpu.hl++, get_a());
}

void ldi_a_hlp(void) {
    set_a(mem_read8(cpu.hl++));
}
...
void ld_nnp_sp(void) {
    ...
    cpu.pc += 2;
    mem_write16(addr, cpu.sp);
}

/* shared by the four ADD HL, rr instructions: sets N/H/C, leaves Z alone */
static void add_hl(uint16_t val) {
    uint32_t result = (uint32_t)cpu.hl + val;

    write_flag(HALF_CARRY_FLAG, ((cpu.hl & 0x0FFF) + (val & 0x0FFF)) > 0x0FFF);
    write_flag(CARRY_FLAG, result > 0xFFFF);
    write_flag(SUBTRACTION_FLAG, 0);

    cpu.hl = (uint16_t)result;
}

void add_hl_bc(void) { add_hl(cpu.bc); }
void add_hl_de(void) { add_hl(cpu.de); }
void add_hl_hl(void) { add_hl(cpu.hl); }
void add_hl_sp(void) { add_hl(cpu.sp); }
